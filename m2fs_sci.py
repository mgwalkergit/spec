...
ccd='b'
i=1
root=datadir+utdate[i]+'/'+ccd+str(scifile[i].split('-')[0]).zfill(4)
...
shite=np.where(data.data>1000)[0]
data.mask[shite]=True
plt.imshow(data.data,vmin=0,vmax=30,cmap='gray')
#plt.imshow(data.data,vmin=100,vmax=1000,cmap='gray')
plt.xlabel('column')
plt.ylabel('row')
plt.xlim([0,2048])
plt.ylim([0,2056])
plt.savefig('m2fs_sci.pdf',dpi=200)
plt.show()
plt.close()
