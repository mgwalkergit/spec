...
fits_list0='all_m2fshires_files'
    ...
    if make_skysub:#write skysub.dat file
        ...
        for j in targets:
            ...
            if len(np.where(fitsobject.mask[j]==False)[0])>0:#write skysub.dat file only if >100 good pixels in spectrum
                ...
                g1=open(out,'w')
                g00.write(out+' '+chains_directory+root[j]+' \n')
                g11.write(out+' \n')
                g12.write(chains_directory+root[j]+'post_equal_weights.dat'+' \n')
...
shite=ascii.write(tab,'kgo_m2fs.ecsv',overwrite=True,format='ecsv',formats={'ra_deg':'12.6f'})
