...
data2='harris2.dat'
data3='harris3.dat'
data4='tuc2_phoenix_gradientpost_equal_weights.dat'
data5='ret2_gradientpost_equal_weights.dat'
data6='cra_gradientpost_equal_weights.dat'
out1='tuc2_phoenix_scaling_newcommands.tex'
g1=open(out1,'w')
...
tuc2_rhalf0=165.
tuc2_sigrhalf0=28.
tuc2_rhalf=np.random.normal(loc=tuc2_rhalf0,scale=1.,size=len(tuc2vdisp))
#tuc2_sigrhalf=np.array([1.])
tuc2_absvmag0=-3.8
tuc2_absvmag=np.random.normal(loc=tuc2_absvmag0,scale=0.1,size=len(tuc2vdisp))
...
ax1_2.scatter(np.median(tuc2_absvmag),tuc2_feh,s=20,lw=0,edgecolor='none',alpha=0.99,marker='s',color='k',rasterized=True)
...
ax1_2.errorbar(np.median(tuc2_absvmag),tuc2_feh,xerr=np.std(tuc2_absvmag),yerr=tuc2_sigfeh,elinewidth=1,fmt='o',capsize=0,alpha=1,color='k',rasterized=True)
...
ax1_2.text(np.median(tuc2_absvmag)-0.15,tuc2_feh[0]-0.4,'Tuc 2',fontsize=7,color='k')
...
#ax1_1.scatter(np.median(tuc2_absvmag),np.median(tuc2_mlratio),s=20,lw=0,edgecolor='none',alpha=0.99,marker='s',color='b',rasterized=True,label='Ret 2')
ax1_1.scatter(np.median(tuc2_absvmag0),np.median(tuc2_mlratio0),s=20,lw=0,edgecolor='none',alpha=0.99,marker='s',color='k',rasterized=True,label='Tuc 2')
...
#lower_error=np.array([(tuc2_mlratio[tuc2like == max(tuc2like)])[0]-mycode2.hpd(tuc2_mlratio,0.32)[0]])
#upper_error=np.array([mycode2.hpd(tuc2_mlratio,0.32)[1]-(tuc2_mlratio[tuc2like == max(tuc2like)])[0]])       
#print (tuc2_mlratio[tuc2like == max(tuc2like)])[0]
#print lower_error
#print upper_error
...
ax1_1.errorbar(np.median(tuc2_absvmag),np.median(tuc2_mlratio),xerr=np.std(tuc2_absvmag),yerr=[tuc2_lower_error,tuc2_upper_error],elinewidth=1,fmt='.',capsize=0,alpha=1,color='k',rasterized=True)
...
#ax1_1.errorbar([tuc2_absvmag0],[tuc2_mlratio0],xerr=[tuc2_sigabsvmag0],yerr=[tuc2_sigmlratio0],elinewidth=0.25,fmt='.',capsize=0,alpha=1,color='g',rasterized=True)
#ax1_1.text(tuc2_absvmag[0]-0.15,tuc2_mlratio[0]+45,'Ret 2',fontsize=7,color='k')
ax1_1.legend(loc=1,fontsize=6,handlelength=0,numpoints=1,scatterpoints=1,shadow=False)

#maxlikemdyn=1./0.0043*tuc2_rhalf0*tuc2vdisp[(tuc2like == max(tuc2like))[0]]**2
#maxlikemrhalf=2.5/0.0043*tuc2_rhalf0*tuc2vdisp[(tuc2like == max(tuc2like))[0]]**2
#maxlikemldyn=maxlikemdyn/tuc2_luminosity0
#maxlikemlratio=maxlikemrhalf/(tuc2_luminosity0/2.)
#lower_error=np.array(maxlikemldyn-mycode2.hpd(tuc2_mlratio,0.32)[0])
#upper_error=np.array(mycode2.hpd(tuc2_mlratio,0.32)[1]-maxlikemldyn)


g1.write(r'\newcommand{\mrhalf}{$'+str(round(np.median(2.5*tuc2_mrhalf/1.e+5),1))+r'_{'+str(round(np.percentile(2.5*tuc2_mrhalf/1.e+5,16)-np.percentile(2.5*tuc2_mrhalf/1.e+5,50),1))+r'}^{+'+str(round(np.percentile(2.5*tuc2_mrhalf/1.e+5,84)-np.percentile(2.5*tuc2_mrhalf/1.e+5,50),1))+r'}\times 10^5$}'+'\n')
g1.write(r'\newcommand{\mlratio}{$'+str(int(np.median(2.*2.5*tuc2_mlratio)))+r'_{'+str(int(np.percentile(2.*2.5*tuc2_mlratio,16)-np.percentile(2.*2.5*tuc2_mlratio,50)))+r'}^{+'+str(int(np.percentile(2.*2.5*tuc2_mlratio,84)-np.percentile(2.*2.5*tuc2_mlratio,50)))+r'}$}'+'\n')
g1.close()

plotfilename='tuc2_phoenix_scaling.pdf'
