...
apmask=True#mask apertures to create 2d data frame containing only extra-aperture light
apflat=True
apflatcorr=True
scatteredlightcorr=True#fit 2d function to extra-aperture light and subtract from data frame
extract1d_flat=True#extract 1d spectra for flat frames
...
for i in range(0,len(utdate)):
    for ccd in ('r','b'):
        ...
        data_file=root+'_stitched0.fits'
        ...
        apmask_file=root+'_apmask_enoise.pickle'
        apflat_file=root+'_apflat_enoise.pickle'
        apflat_residual_file=root+'_apflat_residual_enoise.pickle'
        extract1d_array_flat_file=root+'_extract1d_array_enoise.pickle'
        thars_array_file=root2+'_thars_array.pickle'
        thars_plot_file=root2+'_thars.pdf'
        throughput_continuum_file=root+'_throughput_continuum_enoise.pickle'
        stack_array_file=root2+'_stack_array_enoise.pickle'
        stack_fits_file=root2+'stackskysub.fits'
        twilightstack_array_file=root2+'_twilightstack_array_enoise.pickle'
...
#        aperture_array=pickle.load(open(aperture_array_file,'rb'))
        ...
        if shite:
            ...
            data=pickle.load(open(root0+'_apflatcorr_enoise.pickle','rb'))
...
#            plt.show()
        ...
        if apflatcorr:
            ...
            for j in range(0,len(allfile0[i])):
                ...
                data_file=root0+'_stitched0.fits'
                apflatcorr_file=root0+'_apflatcorr_enoise.pickle'
        ...
        if scatteredlightcorr:
            ...
            for j in range(0,len(allfile0[i])):
                ...
                data_file=root0+'_stitched0.fits'
                apflatcorr_file=root0+'_apflatcorr_enoise.pickle'
                scatteredlightcorr_file=root0+'_scatteredlightcorr_enoise.pickle'
        ...
        if extract1d_flat:
            ...
            for j in range(0,len(flatfile0[i])):
                ...
                data_file=root0+'_stitched0.fits'
                apflat_file=root0+'_apflat_enoise.pickle'
                apflatcorr_file=root0+'_apflatcorr_enoise.pickle'
                apflat_residual_file=root0+'_apflat_residual_enoise.pickle'
                scatteredlightcorr_file=root0+'_scatteredlightcorr_enoise.pickle'
                extract1d_array_file=root0+'_extract1d_array_enoise.pickle'
        ...
        if id_lines_translate:
            ...
            for j in range(0,len(tharfile0[i])):
                ...
                if (not(id_lines_array_exist))|(id_lines_array_exist & overwrite):
                    ...
                    extract1d_array_template_file=template_root+'_extract1d_array_enoise.pickle'
...
#                plt.show()
        ...
        if cr_reject:
            ...
            for j in range(0,len(scifile0[i])):
                ...
                extract1d_array_exists=path.exists(extract1d_array_file)
                wavcal_array_file=root0+'_wavcal_array.pickle'
                cr_reject_array_file=root0+'_cr_reject_array_enoise.pickle'
        ...
        if throughputcorr:
            ...
            for qqq in range(0,len(utdate)):
                if 'twilight' in field_name[qqq]:
                    ...
                    data_file=datadir+utdate[qqq]+'/'+ccd+str(scifile0[qqq][0]).zfill(4)+'_stitched0.fits'
                    data=astropy.nddata.CCDData.read(data_file)#format is such that data.data[:,0] has column-0 value in all rows
                    filtername=data.header['FILTER']
                    if filtername=='Mgb_Rev2':
                        filtername='Mgb_HiRes'
                    if len(wavcal0)>0:
                        ...
                        twi_name.append(root4+'_twilightstack_array_enoise.pickle')
            ...
            for j in range(0,len(allfile0[i])):
                if ((allfile0[i][j] not in tharfile0[i])&(allfile0[i][j] not in flatfile0[i])):#no need to do this for thars or flats
                    ...
                    times=time.Time(time0,location=lco,precision=6)
                    mjd=np.mean(times.mjd)
                    throughput_array_file=root0+'_throughput_array_enoise.pickle'
                    throughputcorr_array_file=root0+'_throughputcorr_array_enoise.pickle'
                    wavcal_array_file=root0+'_wavcal_array.pickle'
        ...
        if plugmap:
            ...
            if (('twilight' not in field_name[i])&('standard' not in field_name[i])):
                for j in range(0,len(scifile0[i])):
                    ...
                    throughput_array_file=root0+'_throughput_array_enoise.pickle'
                    throughputcorr_array_file=root0+'_throughputcorr_array_enoise.pickle'
                    plugmap_file=root0+'_plugmap_enoise.pickle'
        ...
        if skysubtract:
            ...
            if (('twilight' not in field_name[i])&('standard' not in field_name[i])):
                for j in range(0,len(scifile0[i])):
                    ...
                    plugmap_file=root0+'_plugmap_enoise.pickle'
                    wavcal_array_file=root0+'_wavcal_array.pickle'
                    sky_array_file=root0+'_sky_array_enoise.pickle'
                    skysubtract_array_file=root0+'_skysubtract_array_enoise.pickle'
        ...
        if writefits:
            ...
            if (('twilight' not in field_name[i])&('standard' not in field_name[i])):
                ...
                for j in range(0,len(scifile0[i])):
                    ...
                    fits_file=root0+'_'+hjdstring+'_skysubtract.fits'
                ...
                hjdstring=str(round(new_hdul[5].data['hjd'][0],2))
                stack_fits_file=root2+'_'+hjdstring+'_stackskysub.fits'
                stack_fits_exists=path.exists(stack_fits_file)
