...
crapid_lines_translate_stage2=False
id_lines_translate_stage2=True
...
#                    pickle.dump(id_lines_array,open(id_lines_array_file,'wb'))
        ...
        if tharcheck:
            ...
            if (not(thars_array_exists))|(thars_array_exists & overwrite):
                ...
                if len(thars_array)>0:
                    if len(thars_array[0].temperature)>0:
                        order=np.argsort(thars_array[0].temperature)
                        print(order)
                for q in range(0,len(thars_array)):
                    ...
                    cb2=ax2.scatter(thars_array[q].wav,np.full(len(thars_array[q].wav),thars_array[q].aperture),c=thars_array[q].wav_func_std,s=3,cmap='inferno',vmin=0,vmax=0.05)
                    if len(thars_array)>0:
                        if len(thars_array[0].temperature)>0 and len(thars_array[q].wav1024)==len(thars_array[q].temperature):
                            ax3.plot(thars_array[q].temperature[order],thars_array[q].wav1024[order]-thars_array[q].wav1024[order][0],alpha=0.1,color='k')
