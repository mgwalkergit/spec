...
#                    plt.plot(x,y,color='r',alpha=1,lw=0.5)
            apertures_profile_middle,middle_column=pickle.load(open(apertures_profile_middle_file,'rb'))
            print('checking for fibermap files to compare with ',root)
            ...
            if len(map_file)>0:
                for j in range(0,len(map_file)):
                    maproot=datadir+map_utdate[j]+'/'+ccd+'_stackfibermap'+str(j).zfill(1)
                    data_file=datadir+map_utdate[j]+'/'+ccd+str(mapfile0[j][0]).zfill(4)+'_stitched.fits'
                    map_data=astropy.nddata.CCDData.read(data_file)#format is such that data.data[:,0] has column-0 value in all rows
#                    map_data=pickle.load(open(data_file,'rb'))
                    map_filtername=map_data.header['FILTER']
                    if map_filtername=='Mgb_Rev2':
                        map_filtername='Mgb_HiRes'
                    if filtername==map_filtername:
                        map_columnspec_array_file=maproot+'_columnspec_array.pickle'
                        map_apertures_profile_middle_file=maproot+'_apertures_profile_middle.pickle'
                        map_image_boundary_file=maproot+'_image_boundary.pickle'
                        map_aperture_array_file=maproot+'_aperture_array.pickle'
                        mapflat_compare_file=root+'_mapflat_compare'+str(j).zfill(1)+'.pickle'
                        mapflat_compare_exists=path.exists(mapflat_compare_file)
                        print('comparing to ',maproot)
                        if (not(mapflat_compare_exists))|(mapflat_compare_exists & overwrite):
                            if overwrite:
                                print('will overwrite existing')
                            map_image_boundary=pickle.load(open(map_image_boundary_file,'rb'))
                            map_columnspec_array=pickle.load(open(map_columnspec_array_file,'rb'))
                            map_apertures_profile_middle,map_middle_column=pickle.load(open(map_apertures_profile_middle_file,'rb'))
                            map_aperture_array=pickle.load(open(map_aperture_array_file,'rb'))
                            mapflat_compare0=m2fs.get_mapflat_compare(apertures_profile_middle,map_apertures_profile_middle,mapflat_compare_iterations,mapflat_compare_order)
                            pickle.dump(mapflat_compare0,open(mapflat_compare_file,'wb'))
#                            for k in range(0,len(map_aperture_array)):
#                                print(k)
#                                if map_aperture_array[k].trace_npoints>40:
#                                    pos1=apertures_profile_middle.fit[k].mean.quantity.value
#                                    pos2=mapflat_compare0.func(pos1)
#                                    x=np.linspace(map_aperture_array[k].trace_pixel_min,map_aperture_array[k].trace_pixel_max,100)
#                                    y=map_aperture_array[k].trace_func(x)+pos1-pos2
...
#            plt.show()
        ...
        if apflat:
            ...
            if (not(apflat_exists))|(apflat_exists & overwrite):
                ...
                apflat0,apflat_residual,flat_model=m2fs.get_apflat(data,aperture_array,apertures_profile_middle,aperture_peak,image_boundary,apmask0,field_name[i])
...
#                    np.pause()
        ...
        if crapid_lines_translate:
            ...
            for j in range(0,len(tharfile0[i])):
                ...
                if (not(id_lines_array_exist))|(id_lines_array_exist):
...
#                        print('working on aperture ',j)
#                        new_id=m2fs.get_id_lines_translate(extract1d_array_template[0],id_lines_template0,extract1d_array[j],linelist,id_lines_continuum_rejection_low,id_lines_continuum_rejection_high,id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_order,id_lines_threshold_factor,id_lines_window,id_lines_order,id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_sigma,id_lines_tol_angs,id_lines_tol_pix,resolution_order,resolution_rejection_iterations,id_lines_translate_add_lines_iterations)
                        shite=np.where(extract1d_array[j].aperture==np.array([id_lines_array[q].aperture for q in range(0,len(id_lines_array))]))[0][0]
                        if shite>0:
                            new_id=id_lines_array[j]
                            if new_id.npoints<id_lines_minlines:
                                print(j,extract1d_array[j].aperture,id_lines_array[shite].aperture,id_lines_array[shite].npoints,' ddddd')
#                                new_id=m2fs.get_id_lines_translate(extract1d_array_template[0],id_lines_template0,extract1d_array[j],linelist,id_lines_continuum_rejection_low,id_lines_continuum_rejection_high,id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_order,id_lines_threshold_factor,id_lines_window,id_lines_order,id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_sigma,id_lines_tol_angs,id_lines_tol_pix,resolution_order,resolution_rejection_iterations,id_lines_translate_add_lines_iterations)
                                map_new_id=m2fs.get_id_lines_translate(extract1d_array_template[0],id_lines_template0,map_extract1d_array[j],linelist,id_lines_continuum_rejection_low,id_lines_continuum_rejection_high,id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_order,id_lines_threshold_factor,id_lines_window,id_lines_order,id_lines_continuum_rejection_iterations,id_lines_continuum_rejection_sigma,id_lines_tol_angs,id_lines_tol_pix,resolution_order,resolution_rejection_iterations,id_lines_translate_add_lines_iterations)
                                print(new_id.npoints,map_new_id.npoints,' bbbbbb')
#                    np.pause()
