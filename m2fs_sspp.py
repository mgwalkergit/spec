...
fits_list0='sex_m2fshiressspp_files'
...
for i in range(0,len(fitsfile)):
    ...
    for j in range(0,len(fitsobject.obj)):
        root.append('m2fs_sspp_'+m2fsrun+'_'+obj[i]+'_'+field_name+'_'+filtername+'_ra'+str.format('{0:.6f}',round(fitsobject.radeg[j],6)).zfill(6)+'_dec'+str.format('{0:.6f}',round(fitsobject.decdeg[j],6)).zfill(6)+'_hjd'+str.format('{0:.3f}',round(fitsobject.hjd[j],3))+'_ap'+fitsobject.channel[j]+str(int(fitsobject.aperture[j])).zfill(3))
        root2.append('m2fs_sspp_'+m2fsrun+'_'+obj[i]+'_'+field_name+'_'+filtername+'_ra'+str.format('{0:.6f}',round(fitsobject.radeg[j],6)).zfill(6)+'_dec'+str.format('{0:.6f}',round(fitsobject.decdeg[j],6)).zfill(6)+'_hjd'+str.format('{0:.3f}',round(fitsobject.hjd[j],3))+'_ap'+fitsobject.channel[j]+str(int(fitsobject.aperture[j])).zfill(3))
    root=np.array(root)
...
#                root2.append('m2fs_sspp_'+shite[1])
#            root2=np.array(root2)
            multinest=m2fs.m2fs_multinest_sspp(fit_directory,root2,targets,fitsobject,len(fitsobject.wav[0]))#object containing sample of posterior, moments thereof, bestfit wavelength and bestfit spectrum
            ...
            col7=fits.Column(name='posterior_moments',format='68D',dim='(16,4)',array=multinest.moments)
            col8=fits.Column(name='posterior_1000',format='17000D',dim='(16,1000)',array=multinest.posterior_1000)
...
col48=fits.Column(name='fits_filename',format='A200',array=postfit_filename[targets])
col49=fits.Column(name='fits_index',format='I',array=postfit_index[targets])
