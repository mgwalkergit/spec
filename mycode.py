...
def weightedmean(x,sigx):
    ...
    mean=sum1/sum2
    sum3=np.sum((x-mean)**2/sigx**2)
    return mean,np.sqrt(1./sum2),np.sqrt(sum3/sum2)
...
def coordstring(ra_deg,dec_deg):
    if ra_deg<10.:
        rastring='$00'+f'{ra_deg:.6f}'
    elif ra_deg<100.:
        rastring='$0'+f'{ra_deg:.6f}'
    else:
        rastring='$'+f'{ra_deg:.6f}'
    if dec_deg<0:
        if np.abs(dec_deg)<10.:
            decstring='$-00$'+f'{np.abs(dec_deg):.6f}'
        elif np.abs(dec_deg)<100.:
            decstring='$-0$'+f'{np.abs(dec_deg):.6f}'
        else:
            decstring='$-$'+f'{np.abs(dec_deg):.6f}'
    else:
        if np.abs(dec_deg)<10.:
            decstring='$+00$'+f'{np.abs(dec_deg):.6f}'
        elif np.abs(dec_deg)<100.:
