...
ax1_2.scatter(np.median(ret2_absvmag),ret2_feh,s=20,lw=0,edgecolor='none',alpha=0.99,marker='s',color='k',rasterized=True)
...
ax1_2.text(np.median(ret2_absvmag)-0.15,ret2_feh[0]-0.4,'Ret 2',fontsize=7,color='k')
...
#ax1_1.scatter(np.median(ret2_absvmag),np.median(ret2_mlratio),s=20,lw=0,edgecolor='none',alpha=0.99,marker='s',color='b',rasterized=True,label='Ret 2')
ax1_1.scatter(np.median(ret2_absvmag0),np.median(ret2_mlratio0),s=20,lw=0,edgecolor='none',alpha=0.99,marker='s',color='k',rasterized=True,label='Ret 2')

#lower_error=np.array([(ret2_mlratio[ret2like == max(ret2like)])[0]-mycode2.hpd(ret2_mlratio,0.32)[0]])
#upper_error=np.array([mycode2.hpd(ret2_mlratio,0.32)[1]-(ret2_mlratio[ret2like == max(ret2like)])[0]])       
#print (ret2_mlratio[ret2like == max(ret2like)])[0]
#print lower_error
#print upper_error
...
ax1_1.errorbar(np.median(ret2_absvmag),np.median(ret2_mlratio),xerr=np.std(ret2_absvmag),yerr=[lower_error,upper_error],elinewidth=1,fmt='.',capsize=0,alpha=1,color='k',rasterized=True)
#ax1_1.errorbar([ret2_absvmag0],[ret2_mlratio0],xerr=[ret2_sigabsvmag0],yerr=[ret2_sigmlratio0],elinewidth=0.25,fmt='.',capsize=0,alpha=1,color='g',rasterized=True)
#ax1_1.text(ret2_absvmag[0]-0.15,ret2_mlratio[0]+45,'Ret 2',fontsize=7,color='k')
ax1_1.legend(loc=1,fontsize=6,handlelength=0,numpoints=1,scatterpoints=1,shadow=False)

#maxlikemdyn=1./0.0043*ret2_rhalf0*ret2vdisp[(ret2like == max(ret2like))[0]]**2
#maxlikemrhalf=2.5/0.0043*ret2_rhalf0*ret2vdisp[(ret2like == max(ret2like))[0]]**2
#maxlikemldyn=maxlikemdyn/ret2_luminosity0
#maxlikemlratio=maxlikemrhalf/(ret2_luminosity0/2.)
#lower_error=np.array(maxlikemldyn-mycode2.hpd(ret2_mlratio,0.32)[0])
#upper_error=np.array(mycode2.hpd(ret2_mlratio,0.32)[1]-maxlikemldyn)
