...
data_out='all_m2fs.dat'
fits_table_filename='all_m2fs.fits'
...
dev=[]
for i in range(100,len(fitsfile)):
    hdul=fits.open(fitsfile[i])
    res=[]
    npoints=[]
    for j in range(0,len(hdul[5].data['wav_resolution'])):
        res.append(np.mean(hdul[5].data['wav_resolution'][j]))
        npoints.append(np.mean(hdul[5].data['wav_npoints'][j]))
    res=np.array(res)
    npoints=np.array(npoints)
    ap=hdul[5].data['aperture']
    plt.scatter(ap[npoints>35],res[npoints>35],s=1)
    plt.ylim([0.05,0.12])
    plt.show()
    plt.close()
    np.pause()
...
#                np.pause()
    if compile_fits:#process posterior files and put results into new fits file
        ...
        if len(fitsobject.obj)>0:
            ...
            col13=fits.Column(name='wav_npoints',format='A100',array=fitsobject.wav_npoints)
            col14=fits.Column(name='wav_rms',format='A100',array=fitsobject.wav_rms)
            col15=fits.Column(name='temperature',format='A100',array=fitsobject.temperature)
...
col32=fits.Column(name='n',format='D',array=postfit_n[targets])
col33=fits.Column(name='run_id',format='A100',array=postfit_run_id[targets])
col34=fits.Column(name='field_name',format='A100',array=postfit_field_name[targets])
col35=fits.Column(name='wav_npoints',format='A100',array=postfit_wav_npoints[targets])
col36=fits.Column(name='wav_rms',format='A100',array=postfit_wav_rms[targets])
col37=fits.Column(name='temperature',format='A100',array=postfit_temperature[targets])
...
plt.savefig('all_m2fs.pdf',dpi=200)
