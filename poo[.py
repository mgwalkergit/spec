...
with open(directory+m2fsrun+'_10_science_raw') as f: 
    data=f.readlines()[0:]
for line in data:
    p=line.split()
    if p[0]!='none':
...
if id_lines_check:
    ...
    for i in range(0,len(arcfilename)):
        ...
        extract1d_array_file=root+'_extract1d_array.pickle'
        id_lines_array_file=root+'_id_lines_array_noflat.pickle'
...
for i in range(0,len(utdate)):
    for ccd in ('r','b'):
        ...
        extract1d_array_flat_file=root+'_extract1d_array_noflat.pickle'
        thars_array_file=root2+'_thars_array_noflat.pickle'
        thars_plot_file=root2+'_thars_noflat.pdf'
        throughput_continuum_file=root+'_throughput_continuum_noflat.pickle'
        stack_array_file=root2+'_stack_array_noflat.pickle'
        stack_fits_file=root2+'stackskysub_noflat.fits'
        twilightstack_array_file=root2+'_twilightstack_array_noflat.pickle'
        twilightstack_fits_file=root2+'_twilightstack_noflat.fits'
        stack_skysub_file=root2+'_stackskysub_noflat.dat'
...
#            if apertures_profile_middle.realvirtual[j]:
        ...
        if apflatcorr:
            ...
            for j in range(0,len(allfile0[i])):
                ...
                apflatcorr_file=root0+'_apflatcorr_noflat.pickle'
                ...
                if (not(apflatcorr_exists))|(apflatcorr_exists & overwrite):
                    ...
                    apflatcorr0=data.divide(apflat0_noflat)
        ...
        if scatteredlightcorr:
            ...
            for j in range(0,len(allfile0[i])):
                ...
                data_file=root0+'_stitched.fits'
                apflatcorr_file=root0+'_apflatcorr_noflat.pickle'
                scatteredlightcorr_file=root0+'_scatteredlightcorr_noflat.pickle'
        ...
        if extract1d_flat:
            ...
            for j in range(0,len(flatfile0[i])):
                ...
                scatteredlightcorr_file=root0+'_scatteredlightcorr_noflat.pickle'
                extract1d_array_file=root0+'_extract1d_array_noflat.pickle'
        ...
        if id_lines_translate:
            ...
            for j in range(0,len(tharfile0[i])):
                ...
                if (not(id_lines_array_exist))|(id_lines_array_exist & overwrite):
                    ...
                    extract1d_array_template_file=template_root+'_extract1d_array_noflat.pickle'
...
#                plt.show()
        ...
        if wavcal:
            ...
            for j in range(0,len(allfile0[i])):
                if allfile0[i][j] not in tharfile0[i]:#no need to do this for thars
                    ...
                    data_file=root0+'_stitched.fits'
                    extract1d_array_file=root0+'_extract1d_array_noflat.pickle'
                    wavcal_array_file=root0+'_wavcal_array_noflat.pickle'
        ...
        if cr_reject:
            ...
            for j in range(0,len(scifile0[i])):
                ...
                extract1d_array_exists=path.exists(extract1d_array_file)
                wavcal_array_file=root0+'_wavcal_array_noflat.pickle'
                cr_reject_array_file=root0+'_cr_reject_array_noflat.pickle'
        ...
        if stack_twilight:
            ...
            if 'twilight' in field_name[i]:
                ...
                if (not(twilightstack_array_exists))|(twilightstack_array_exists & overwrite):
                    ...
                    twilightstack_wavcal_array_file=root2+'_twilightstack_wavcal_array_noflat.pickle'
...
#                    wavcal_array=pickle.load(open(wavcal_array_file,'rb'))
#                    id_lines_array_file=datadir+utdate[i]+'/'+ccd+str(tharfile0[i][j]).zfill(4)+'_id_lines_array_noflat.pickle'
#                    id_lines_array=pickle.load(open(id_lines_array_file,'rb'))
                        data=astropy.nddata.CCDData.read(data_file)#format is such that data.data[:,0] has column-0 value in all rows
        ...
        if throughputcorr:
            ...
            for qqq in range(0,len(utdate)):
                if 'twilight' in field_name[qqq]:
                    ...
                    wavcal0=pickle.load(open(root4+'_twilightstack_wavcal_array_noflat.pickle','rb'))
                    ...
                    if filtername=='Mgb_Rev2':
                        filtername='Mgb_HiRes'
                    if len(wavcal0)>0:
                        twi_wavcal_name.append(root4+'_twilightstack_wavcal_array_noflat.pickle')
                        twi_name.append(root4+'_twilightstack_array_noflat.pickle')
            ...
            for j in range(0,len(allfile0[i])):
                if ((allfile0[i][j] not in tharfile0[i])&(allfile0[i][j] not in flatfile0[i])):#no need to do this for thars or flats
                    ...
                    times=time.Time(time0,location=lco,precision=6)
                    mjd=np.mean(times.mjd)
                    throughput_array_file=root0+'_throughput_array_noflat.pickle'
                    throughputcorr_array_file=root0+'_throughputcorr_array_noflat.pickle'
                    wavcal_array_file=root0+'_wavcal_array_noflat.pickle'
                    ...
                    if len(keep)>0:
                        ...
                        if (not(throughputcorr_array_exists))|(throughputcorr_array_exists & overwrite):
                            ...
                            print(len(cr_reject_array))
                            for k in range(0,len(cr_reject_array)):
                                print('working on ',k+1,' of ',len(cr_reject_array))

                                throughput,throughputcorr=m2fs.get_throughputcorr(k,throughput_continuum,cr_reject_array,twilightstack_array,twilightstack_continuum_array,wavcal_array,twilightstack_wavcal_array)
...
#                            ax2.plot(cr_reject_array[k].spec1d_flux[cr_reject_array[k].spec1d_mask==False],color='r',lw=0.1,alpha=0.1)
#                        ax1.set_ylim([0,30000])
#                        ax2.set_ylim([0,30000])
#                        plt.show()
#                        plt.close()
        ...
        if plugmap:
            if (('twilight' not in field_name[i])&('standard' not in field_name[i])):
                for j in range(0,len(scifile0[i])):
                    ...
                    throughput_array_file=root0+'_throughput_array_noflat.pickle'
                    throughputcorr_array_file=root0+'_throughputcorr_array_noflat.pickle'
                    plugmap_file=root0+'_plugmap_noflat.pickle'
                    plugmap_exists=path.exists(plugmap_file)
                    id_lines_array_file=datadir+utdate[i]+'/'+ccd+str(tharfile0[i][0]).zfill(4)+'_id_lines_array_noflat.pickle'
                    print('plugmap for: \n'+root0)
...
#                    id_lines_array_file=datadir+utdate[i]+'/'+ccd+str(tharfile0[i][0]).zfill(4)+'_id_lines_array_noflat.pickle'
                    plugmap_file=root0+'_plugmap_noflat.pickle'
                    wavcal_array_file=root0+'_wavcal_array_noflat.pickle'
                    sky_array_file=root0+'_sky_array_noflat.pickle'
                    skysubtract_array_file=root0+'_skysubtract_array_noflat.pickle'
        ...
        if stack_frames:
            ...
            if (('twilight' not in field_name[i])&('standard' not in field_name[i])):
                ...
                if (not(stack_array_exists))|(stack_array_exists & overwrite):
                    ...
                    stack_wavcal_array_file=root2+'_stack_wavcal_array_noflat.pickle'
                    ...
                    for k in range(0,len(scifile0[i])):
                        ...
                        for q in range(0,len(throughput_array)):
                            if len(wavcal_array[q].wav)>0:
                                ax3.plot(wavcal_array[q].wav[throughputcorr_array[q].spec1d_mask==False],throughput_array[q].value[throughputcorr_array[q].spec1d_mask==False],alpha=0.2,lw=0.3)
            ...
            if (('twilight' not in field_name[i])&('standard' not in field_name[i])):
                if (not(stack_fits_exists))|(stack_fits_exists & overwrite):
                    ...
                    for j in range(0,len(scifile0[i])):
                        ...
                        skysubtract_array_file=root0+'_skysubtract_array_noflat.pickle'
                        sky_array_file=root0+'_sky_array_noflat.pickle'
                        fits_file=root0+'_skysubtract_noflat.fits'
                        plugmap_file=root0+'_plugmap_noflat.pickle'
                        ...
                        new_hdul=m2fs.get_hdul(data,skysubtract_array,sky_array,wavcal_array,plugmap0,m2fsrun,field_name[i])
...
#                stack_mjd=stack_wavcal_array[0].mjd
                    new_hdul=m2fs.get_hdul(data,stack_array,sky_array,stack_wavcal_array,plugmap0,m2fsrun,field_name[i])
                    new_hdul[0].header['filtername']=filtername
                    new_hdul[0].header['m2fsrun']=m2fsrun
                    new_hdul[0].header['field_name']=field_name[i]
