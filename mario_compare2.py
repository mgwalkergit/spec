...
ax1.scatter(v1,v2[this],color='k',s=1)
ax1.errorbar(v1,v2[this],xerr=sigv1,yerr=sigv2[this],fmt='.',capsize=0,mew=0.1,color='k',lw=0.3)
ax1.plot([-1000,1000],[-1000,1000],lw=1,linestyle=':',color='k')
ax1.set_xlim([-130,-115])
ax1.set_ylim([-130,-115])
ax1.set_xlabel('velocity from mattraf [km/s]',fontsize=8)
ax1.set_ylabel('velocity from marioraf [km/s]',fontsize=8)

ax2.scatter(sigv1,sigv2[this],color='k',s=1)
ax2.plot([-1000,1000],[-1000,1000],lw=1,linestyle=':',color='k')
ax2.set_xlim([0,5])
ax2.set_ylim([0,5])
ax2.set_xlabel('velocity error from mattraf [km/s]',fontsize=8)
ax2.set_ylabel('velocity error from marioraf [km/s]',fontsize=8)

plt.savefig('mario_compare2.pdf',dpi=200)
plt.show()
plt.close()
