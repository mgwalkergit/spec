...
with open(directory+'crapall_dark_raw') as f:
    data=f.readlines()[0:]
m2fsrun=[]
utdate=[]
file1=[]
...
for ccd in (['b','r']):
    for chip in (['c1','c2','c3','c4']):
        ...
        master_debiased=[]
        master_exptime=[]
        for j in range(0,len(m2fsrun)):
            print(directory+m2fsrun[j]+'_'+ccd+'_'+chip+'_master_bias.fits')
            master_bias=astropy.nddata.CCDData.read(directory+m2fsrun[j]+'_'+ccd+'_'+chip+'_master_bias.fits')
